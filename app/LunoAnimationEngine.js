class LunoAnimationEngine {
  ...
  static wavePulse(element, color) {
    if (!element || !element.getBoundingClientRect) return;
    var elColor = color || '#3fb950';
    var rect = element.getBoundingClientRect();

    var wave = document.createElement('div');
    wave.className = 'luno-wave-ring';
    wave.style.cssText = [
      'position: fixed;',
      'z-index: 99998;',
      'pointer-events: none;',
    ...
    requestAnimationFrame(function() {
      wave.style.transform = 'scale(1.06, 1.15)';
      wave.style.opacity = '0';
      setTimeout(function() { if (wave.parentNode) wave.remove(); }, 660);
    });
  }

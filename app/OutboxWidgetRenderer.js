class OutboxWidgetRenderer {
  ...
  static renderWidget(containerId, retryCount) {
    ...
    try {
      ...
      actionRow.style.cssText = 'display:flex; gap:0.5rem; margin-bottom:0.6rem; flex-wrap:wrap;';

      var bundleBtn = document.createElement('button');
      bundleBtn.style.cssText = 'flex:1; padding:0.75rem; background:#8257e5; color:#fff; border:none; border-radius:8px; font-weight:bold; font-size:0.92rem; cursor:pointer; font-family:monospace; box-shadow:0 4px 12px rgba(130, 87, 229, 0.3);';
      ...
      writePromptBtn.style.cssText = 'flex:1; padding:0.75rem; background:#271052; color:#d2a8ff; border:1px solid #8257e5; border-radius:8px; font-size:0.92rem; font-weight:bold; cursor:pointer; font-family:monospace;';
      writePromptBtn.textContent = '✍️ Write Prompt';
      writePromptBtn.onclick = function() {
        if (typeof OutboxPromptBox !== 'undefined') {
          OutboxPromptBox.promptWriteNoteModal();
        } else if (targetQueue && targetQueue.promptWriteNoteModal) {
          targetQueue.promptWriteNoteModal();
        }
      };
      ...
      mainCopyBtn.style.cssText = 'width:100%; padding:0.75rem; font-weight:bold; border-radius:8px; font-size:0.9rem; cursor:pointer; font-family:monospace; margin-bottom:0.5rem; background:' + (itemCount > 0 ? '#8257e5' : '#21262d') + '; color:' + (itemCount > 0 ? '#fff' : '#8b949e') + '; border:1px solid ' + (itemCount > 0 ? '#8257e5' : '#30363d') + ';';
      mainCopyBtn.textContent = itemCount > 0 ? ('📋 Copy Outbox Package (' + itemCount + ' item' + (itemCount === 1 ? '' : 's') + ')') : '📋 Copy Outbox (Empty)';
      mainCopyBtn.onclick = function() {
        if (targetQueue && targetQueue.copyPackageToClipboard) {
